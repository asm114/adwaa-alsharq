...
function dueItems(){
  const date=today(),rows=bookings().filter(b=>b.status!=='ملغي'&&b.recordType!=='family');
  const todayRows=rows.filter(b=>b.date===date);
  const entryPending=todayRows.filter(b=>!['تم الدخول','تم الخروج'].includes(b.status));
  const exitPending=todayRows.filter(b=>b.status==='تم الدخول');
  const dueMoney=rows.filter(b=>Number(b.total||0)>Number(b.paid||0));
  const commissions=rows.filter(b=>b.commissionSnapshot?.status==='earned');
  const cleaning=(state().cleaningTasks||[]).filter(t=>t.status!=='done'&&t.status!=='ملغي');
  return {todayRows,entryPending,exitPending,dueMoney,commissions,cleaning};
}
