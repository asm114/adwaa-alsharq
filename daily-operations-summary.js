...
function dueItems(){const date=today(),rows=bookings().filter(b=>!['ملغي','مؤجل'].includes(b.status)&&b.recordType!=='family'),todayRows=rows.filter(b=>b.date===date),entryPending=todayRows.filter(b=>!['تم الدخول','تم الخروج'].includes(b.status)),exitPending=todayRows.filter(b=>b.status==='تم الدخول'),dueMoney=rows.filter(b=>remaining(b)>0),commissions=rows.filter(b=>b.commissionSnapshot?.status==='earned');return {todayRows,entryPending,exitPending,dueMoney,commissions}}
