...
function outstandingBookings(){
  return (Array.isArray(db?.bookings)?db.bookings:[])
    .filter(b=>statusOf(b)==='earned'&&b?.status!=='ملغي'&&b?.recordType!=='family')
...
function addCommissionReminderNotification(b){
  ...
  const message=`عمولة الحجز #${b.code||''} مستحقة بقيمة ${moneyText(amountOf(b))} ولم يتم تأكيد تحويلها إلى حسابك الخاص.`;
...
async function markReceived(id){
  ...
  if(!b||statusOf(b)!=='earned')return closePrompt(true);
