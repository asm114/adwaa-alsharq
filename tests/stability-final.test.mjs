...
test('Service Worker يستخدم كاش الإصدار الحالي ويحدث fallback عند نجاح فتح التطبيق',async()=>{
  ...
  assert.match(sw,/CACHE=`\$\{CACHE_NAMESPACE\}app-state-20260923-issue150-2`/);
  assert.match(sw,/subscription-financial-core\.js\?v=20260922-1/);
  assert.match(sw,/operational-alert-core\.js\?v=20260922-1/);
  assert.match(sw,/subscription-owner-confirmed-reconciliation\.js\?v=20260923-1/);
...
test('لودرات التدقيق النهائي تستخدم أرقام كاش محدثة وبترتيب عمولة الاشتراك الصحيح',async()=>{
  ...
  assert.match(finalAdmin,/subscription-booking-type\.js\?v=20260922-2/);
  ...
  assert.match(subscription,/subscription-commission-core\.js\?v=20260922-2/);
  assert.match(subscription,/subscription-revenue-integration\.js\?v=20260922-2/);
  assert.match(subscription,/commission-transfer-workflow\.js\?v=20260922-2/);
  assert.match(subscription,/professional-ui-stable\.js\?v=20260922-2/);
