...
test('Commercial unavailable periods are fresh-install safe and booking-sync compatible',async()=>{
  ...
  assert.match(sql,/public reads customer portal unavailable periods/i);
  ...
  assert.doesNotMatch(sql,/update\s+public\.customer_portal_unavailable_periods/i);
  assert.doesNotMatch(sql,/updated_by/i);
});
